...
async def scan_label(task_id: str, payload: ScanLabelIn, request: Request) -> Dict[str, Any]:
    ...
    from services.receiving_roll_service import create_label_roll
    res = await create_label_roll(task=task, decoded=decoded, sitem=sitem, product=product, po=po, sup=sup,
                                  supplier_id=supplier_id, owner_entity_id=owner_entity_id, actor=actor,
                                  manual=manual)
